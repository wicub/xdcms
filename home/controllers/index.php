<?php
	...
	class index extends CI_Controller{
		...
		public function news($num=''){
			...
			//分页
			$this->load->library('pagination');
			$config['base_url']='index.php/index/news';
			$config['total_rows']=$this->db->count_all('news'); //总数
			...
			$this->pagination->initialize($config);//配置分页
			$query=$this->db->get('news',$config['per_page'],$num)->result();
			$data=array('list'=>$query,'v'=>$contact);
			//var_dump($data);exit;
		...
		public function down($num=''){
			...
			//分页
			$this->load->library('pagination');
			$config['base_url']='index.php/index/down';
